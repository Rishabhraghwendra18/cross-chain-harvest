...
async function deployVault() {
    const mooBnMAddress = await deployERC20();
    console.log("Deployging BeefyBnMVault....");
  const BeefyBnMVault = await hre.ethers.getContractFactory("BeefyBnMVault");
  const beefyBnMVault = await BeefyBnMVault.deploy(mooBnMAddress,hre.ethers.utils.parseEther("10"),BNM_TOKEN);
  await beefyBnMVault.deployed();

  console.log(
    `beefyBnMVault is deployed at : ${beefyBnMVault.address}`
  );
  await beefyBnMVault.deployTransaction.wait(5);
  console.log("Verifying contract....");
  await verifyContract(beefyBnMVault.address,[mooBnMAddress,hre.ethers.utils.parseEther("10"),BNM_TOKEN]);
